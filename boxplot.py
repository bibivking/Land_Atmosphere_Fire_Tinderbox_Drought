...
def plot_boxplot(fileout_name,var_name,xyhue=['year','values','pft'],year=None,pft=None,season=None,case=None):
    ...
    if var_name in ["EF","FWsoil_tavg","Albedo_inst"]:
        ylabel  = "(-)"
    ...
    # Plotting boxplot
    sns.boxplot(x=xyhue[0], y=xyhue[1], data=df, ax=axs,showfliers=False,hue=xyhue[2]) # ["m", "g"], palette=["b"], whis=0

    fig.savefig("./plots/boxplot_Tinderbox_"+message+".png", bbox_inches='tight', dpi=300, pad_inches=0.1) #
...
if __name__ == "__main__":
    ...
    if 1:
        ...
        file_paths_ctl = [ LIS_path_ctl+"LIS.CABLE.201701-202006_ALB_LAI.nc" ]

        file_paths_sen = [ LIS_path_sen+"LIS.CABLE.201701-202006_ALB_LAI.nc" ]
        ...
        if 1:
            var_names      = ["Albedo_inst","LAI_inst"]
                            #  ["Tair_f_inst","Wind_f_inst","Qair_f_inst","Psurf_f_inst","SWdown_f_inst","LWdown_f_inst"] 
        ...
        if 0:
            year   = None
            pft    = 2 # [2,5,6,9,14]
            season = 2 # 1: MAM 2:JJA 3:SON 4:DJF
            case   = None
