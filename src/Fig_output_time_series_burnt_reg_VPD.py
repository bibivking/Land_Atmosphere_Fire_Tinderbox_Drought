...
def regrid_to_fire_map_resolution(fire_path, var_in, lat_in, lon_in, loc_lat=None, loc_lon=None, burn=0):
    ...
    # # for VPD, since VPD is calcuated, the default value -9999 can become weird values,
    # # so I set all -9999. as np.nan. 
    # # comment out here for further check
    # var_in     = np.where(np.isnan(var_in), -9999., var_in)
...
def output_time_series_burn_region(var_name, var_unit, file_out, fire_path, wrf_path, file_name, land_ctl_path, land_sen_path,
    ...
    if var_name =='Tmax':
        ...
        time_ctl, Qle_ctl = read_var_multi_file([land_ctl_path +"Qle_tavg/"+ file_name], 'Qle_tavg',
                                                loc_lat=None, loc_lon=None, lat_name=lat_name, lon_name=lon_name)
        time_sen, Qle_sen = read_var_multi_file([land_sen_path +"Qle_tavg/"+ file_name], 'Qle_tavg',
                                                loc_lat=None, loc_lon=None, lat_name=lat_name, lon_name=lon_name)

        var_daily_ctl = time_clip_to_day(time_ctl, Qle_ctl, time_s, time_e, seconds=None)
        var_daily_sen = time_clip_to_day(time_sen, Qle_sen, time_s, time_e, seconds=None)
        ...
        time, Tair_ctl    = read_var_multi_file(tair_ctl_files, "Tair_f_inst", loc_lat=None, loc_lon=None, lat_name=lat_name, lon_name=lon_name)
        time, Tair_sen    = read_var_multi_file(tair_sen_files, "Tair_f_inst", loc_lat=None, loc_lon=None, lat_name=lat_name, lon_name=lon_name)
        time, Qair_ctl    = read_var_multi_file(qair_ctl_files, "Qair_f_inst", loc_lat=None, loc_lon=None, lat_name=lat_name, lon_name=lon_name)
        time, Qair_sen    = read_var_multi_file(qair_sen_files, "Qair_f_inst", loc_lat=None, loc_lon=None, lat_name=lat_name, lon_name=lon_name)
        time_wrf, Pres_ctl_tmp= read_var_multi_file(pres_ctl_files, "slp", loc_lat=None, loc_lon=None, lat_name=lat_name, lon_name=lon_name)
        time_wrf, Pres_sen_tmp= read_var_multi_file(pres_sen_files, "slp", loc_lat=None, loc_lon=None, lat_name=lat_name, lon_name=lon_name)
        ...
        var_daily_ctl     = time_clip_to_day(time, Ctl_tmp, time_s, time_e, seconds=None)
        var_daily_sen     = time_clip_to_day(time, Sen_tmp, time_s, time_e, seconds=None)
...
if __name__ == "__main__":
    ...
    var_name       = "VPD"
    var_unit       = "kPa"
    file_out       = "/g/data/w97/mm3972/scripts/Drought/drght_2017-2019/nc_files/times_series_201909-202002_"+var_name+".nc"
