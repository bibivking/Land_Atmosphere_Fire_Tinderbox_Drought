    ...
    for num, file_path in enumerate(file_paths):
        ...
        for i in np.arange(ntime):
            ...
            if cur_time < next_file_time:
                ...
                # read 0th ~ (i-1)th time steps from the current file
                time_step_stop = i-1
        ...
        if var_name not in ["cape_2d", "cloudfrac"] and len(np.shape(Var_tmp))==4:
            # Extract the pressure and variable
            p       = getvar(ncfile, "pressure",timeidx=ALL_TIMES) # "height_agl"
            print("p",p)
            Var_tmp = interplevel(Var_tmp, p, desiredlev=[1000,950,900,850,700,500,300,100])
            print("Var_tmp",Var_tmp)
...
def make_wrf_nc(file_paths, file_out_path, var_name):
    ...
    if len(var_dims) == 4:
        if var_name == "cape_2d":
            ...
            pres                = f.createVariable('pressure_levels', 'f4', ('bottom_top'))
            pres.standard_name  = "pressure levels"
            pres.units          = "hPa"
            pres[:]             = [1000,950,900,850,700,500,300,100]
